  ...
  animatables.map { it.value }
    ...
    Box(
        ...
        Canvas(
            ...
            when (val shape = target.style.showcaseShape) {
                ShowcaseShape.CIRCLE -> {
                    drawCircle(
                        color = target.style.targetCircleColor,
                        radius = targetRadius * scaleAnimatable.value,
                        center = targetRect.center,
                        blendMode = BlendMode.Xor
                    )
                }

                is ShowcaseShape.RECTANGLE -> {
                    drawRoundRect(
                        color = target.style.targetCircleColor,
                        topLeft = targetRect.topLeft,
                        size = targetRect.size,
                        cornerRadius = CornerRadius(
                            shape.roundCorner.toPx(),
                            shape.roundCorner.toPx()
                        ),
                        blendMode = BlendMode.Xor
                    )
...
sealed class ShowcaseShape {
    ...
    data class RECTANGLE(val roundCorner: Dp = 0.dp) : ShowcaseShape()
}
...
class ShowcaseStyle(
    ...
    val showcaseShape: ShowcaseShape = ShowcaseShape.CIRCLE
) {
    ...
    fun copy(
        ...
        showcaseShape: ShowcaseShape = this.showcaseShape
    ): ShowcaseStyle {
